...
class PHPLinq_Initiator {
	...
	public static function registerProvider($class = null, $reference = null) {
		if (!is_null($class) && $class != '') {
			...
			// Make sure PHPLinq_LinqToObjects is ALWAYS last
			foreach (self::$_registeredProviders as $key => $provider) {
				if ($provider == 'PHPLinq_LinqToObjects') {
					unset(self::$_registeredProviders[$key]);
					self::$_registeredProviders[] = 'PHPLinq_LinqToObjects';
					break;
				}
			}
		}
